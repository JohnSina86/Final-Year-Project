...
def main():
    ...
    try:
        ...
        print("ERROR: 'Transmitter pos' not found!")
        return
    ...
    try:
        ...
        print(f"Loaded {len(positions)} segments")
    except FileNotFoundError:
        print("ERROR: 'geometry.txt' not found!")
        return
    
    try:
        x_current_real, x_current_imag = [], []
    ...
    print(f"  {time.time() - start_inc:.2f}s")
    
    print(f"[GPU] Scattered field...")
    ...
    for batch_start in range(0, N, batch_size):
        ...
        if (batch_end % 100) == 0 or batch_end == N:
            ...
            print(f"  {progress:.0f}% ({batch_end}/{N}), ETA: {eta:.1f}s")
    
    E_field_gpu *= -(eta_k0_4 * delta_s)
    print(f"  {time.time() - start_scatter:.2f}s")
    ...
    print(f"\nRange: [{E_mag.min():.4e}, {E_mag.max():.4e}] V/m")
    ...
    ax.scatter(x_positions[::downsample], y_positions[::downsample], 
               c='cyan', s=30, edgecolors='white', linewidth=0.5, zorder=10)
