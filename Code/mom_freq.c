...
/*******************************************************************************
* L-BRACKET METHOD OF MOMENTS (MoM) SOLVER - FREQUENCY SWEEP VERSION
* 
* Reads frequency from frequency.txt
* Saves optimal omega to omega.txt for frequency sweep analysis
*******************************************************************************/
...
#define OMEGA_MIN 0.1
#define OMEGA_MAX 1.9
#define OMEGA_TEST_ITERS 1000

// Physical constants
#define M_PI 3.14159265358979323846
#define EPSILON_0 8.854e-12
#define MU_0 1.256637e-6
#define SEGMENTS_PER_LAMBDA 100
...
EMParameters initialize_em_parameters(double frequency_ghz) {
    EMParameters params;
    params.frequency = frequency_ghz * 1e9;  // GHz to Hz
...
double complex calculate_mutual_impedance(double x1, double y1, double x2, double y2,
    ...
    return (params.wavenumber * params.impedance / 4.0) * hankel * params.segment_length;
}

double complex calculate_excitation(double seg_x, double seg_y,
                                   Transmitter tx, EMParameters params) {
    double distance = sqrt((seg_x - tx.x) * (seg_x - tx.x) + (seg_y - tx.y) * (seg_y - tx.y));
    double k_times_r = params.wavenumber * distance;
    double J0 = bessel_j0(k_times_r);
    double Y0 = bessel_y0(k_times_r);
    return J0 - I * Y0;
...
void assemble_system(double complex *Z, double complex *V,
    ...
    #pragma omp parallel for
    for (int i = 0; i < N; i++) {
        V[i] = calculate_excitation(geom.x_positions[i], geom.y_positions[i], tx, params);
    }
...
double ssor_iteration(double complex *Z, double complex *V,
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = i + 1; j < N; j++) sum -= Z_row[j] * x_old[j];
    ...
    double max_change = 0.0, max_magnitude = 1e-20;
    #pragma omp parallel
    {
        double local_max_change = 0.0, local_max_mag = 1e-20;
...
double test_omega_value(double complex *Z, double complex *V, int N,
    ...
    for (int iter = 0; iter < test_iterations; iter++) {
        ...
        if ((iter + 1) % 100 == 0 || iter == test_iterations - 1) final_error = error;
...
int find_best_omega(double *omega_values, double *errors, int count,
    ...
    for (int i = 1; i < count; i++) {
        if (errors[i] < errors[best_idx]) best_idx = i;
    }
    return best_idx;
}
...
double optimize_omega_hierarchical(double complex *Z, double complex *V, int N) {
    ...
    for (int i = 0; i < round1_count; i++) omega_r1[i] = OMEGA_MIN + i * 0.1;
    
    int best_r1 = find_best_omega(omega_r1, errors_r1, round1_count, Z, V, N, OMEGA_TEST_ITERS);
    ...
    for (int i = 0; i < round2_count; i++) omega_r2[i] = omega_min_r2 + i * 0.01;
    
    int best_r2 = find_best_omega(omega_r2, errors_r2, round2_count, Z, V, N, OMEGA_TEST_ITERS);
    ...
    for (int i = 0; i < round3_count; i++) omega_r3[i] = omega_min_r3 + i * 0.001;
    
    int best_r3 = find_best_omega(omega_r3, errors_r3, round3_count, Z, V, N, OMEGA_TEST_ITERS);
...
ConvergenceResult solve_ssor(double complex *Z, double complex *V, double complex *x,
    ...
    if (!result.converged) printf("WARNING: Did not converge in %d iterations\n", max_iter);
...
int main(void) {
    ...
    double frequency_ghz = 2.4;  // Default
    FILE *freq_file = fopen("frequency.txt", "r");
    if (freq_file) {
        if (fscanf(freq_file, "%lf", &frequency_ghz) == 1) {
            printf("\n Frequency from file: %.1f GHz\n", frequency_ghz);
        }
        fclose(freq_file);
    } else {
        printf("\nWARNING: frequency.txt not found, using default %.1f GHz\n", frequency_ghz);
    }
    ...
    // Initialize problem
    EMParameters params = initialize_em_parameters(frequency_ghz);
    Geometry geom = create_geometry(strip_length, params);
    
    printf("\nConfiguration:\n");
    printf("  Frequency: %.1f GHz (λ=%.4f m)\n", params.frequency/1e9, params.wavelength);
    ...
    // SAVE OMEGA TO FILE
    FILE *omega_file = fopen("omega.txt", "w");
    if (omega_file) {
        fprintf(omega_file, "%.6f\n", optimal_omega);
        fclose(omega_file);
    }
    
    // STEP 3: Solve system
    printf("\n=== STEP 3: Solve System ===\n");
    ConvergenceResult result = solve_ssor(Z, V, x, N, optimal_omega, CONVERGENCE_TOL, MAX_ITERATIONS);
    ...
    // Save results
    save_results("current_distribution.txt", x, N, params, geom, tx, optimal_omega, result, total_time);
